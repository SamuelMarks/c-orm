...
TEST test_c_orm_execute_params_valid(void) {
    ...
    res = c_orm_execute_params(db, "INSERT INTO test VALUES (?, ?, ?, ?, ?)", params, 5);
    ASSERT_EQ(0, res);
...
static void test_logger_cb(const char* query, double duration_ms, void* user_data) {
    g_log_call_count++;
    g_last_query = query; /* Note: query pointer must remain valid for this to work */
...
TEST test_c_orm_fluent_query(void) {
    ...
    res = c_orm_query_build(q, &sql);
    ASSERT_EQ(0, res);
    ASSERT_STR_EQ("SELECT id, name FROM users WHERE age > 18 ORDER BY name ASC LIMIT 10", sql);
    free(sql);
    sql = NULL;
