...
TEST test_cache_mutex_fail(void) {
#if !defined(_WIN32) && !defined(_WIN64)
  c_orm_query_t *q1;
  setup_mock_db();
  ...
  mock_mutex_lock_fail = 1;
  ASSERT_EQ(C_ORM_ERROR_UNKNOWN, c_orm_finalize_cached(&mock_db, q1));
  mock_mutex_lock_fail = 0;

  c_orm_finalize_cached(&mock_db, q1);

  mock_mutex_lock_fail = 1;
  ASSERT_EQ(C_ORM_ERROR_UNKNOWN, c_orm_disable_statement_caching(&mock_db));
  mock_mutex_lock_fail = 0;
...
TEST test_cache_unlock_coverage(void) {
#if !defined(_WIN32) && !defined(_WIN64)
  c_orm_query_t *q1;
  setup_mock_db();
  c_orm_enable_statement_caching(&mock_db, 1);

  c_orm_prepare_cached(&mock_db, "SELECT 1", &q1);
  c_orm_finalize_cached(&mock_db, q1);

  /* Cover unlock failure in prepare hit */
  mock_mutex_unlock_fail = 1;
  c_orm_prepare_cached(&mock_db, "SELECT 1", &q1);
  mock_mutex_unlock_fail = 0;
  c_orm_finalize_cached(&mock_db, q1);

  /* Cover unlock failure in prepare error (OOM entry) */
  oom_active = 1;
  oom_countdown = 0;
  mock_mutex_unlock_fail = 1;
  c_orm_prepare_cached(&mock_db, "SELECT 2", &q1);
  mock_mutex_unlock_fail = 0;
  oom_active = 0;
  if (q1)
    c_orm_finalize_cached(&mock_db, q1); /* clean up leaked query */

  /* Cover unlock failure in prepare error (OOM sql) */
  oom_active = 1;
  oom_countdown = 1;
  mock_mutex_unlock_fail = 1;
  c_orm_prepare_cached(&mock_db, "SELECT 3", &q1);
  mock_mutex_unlock_fail = 0;
  oom_active = 0;
  if (q1)
    c_orm_finalize_cached(&mock_db, q1); /* clean up leaked query */

  /* Cover unlock failure in finalize success */
  c_orm_prepare_cached(&mock_db, "SELECT 4", &q1);
  mock_mutex_unlock_fail = 1;
  c_orm_finalize_cached(&mock_db, q1);
  mock_mutex_unlock_fail = 0;

  /* Cover unlock failure on successful miss (caching execution) */
  mock_mutex_unlock_fail = 1;
  c_orm_prepare_cached(&mock_db, "SELECT 6", &q1);
  mock_mutex_unlock_fail = 0;
  c_orm_finalize_cached(&mock_db, q1);

  /* Cover unlock failure when query is NOT in cache */
  c_orm_prepare_cached(
      &mock_db, "SELECT 7",
      &q1); /* this will be evicted soon maybe? no capacity is 1 */
  c_orm_prepare_cached(&mock_db, "SELECT 8",
                       &q1); /* evicts SELECT 7, wait, both not released so
                                SELECT 8 is in cache, SELECT 7 is unmanaged? */
                             /* Cover unlock and destroy failure in disable */
  mock_mutex_unlock_fail = 1;
  ...
  /* Unmanaged query finalize fail */
  c_orm_prepare_cached(&mock_db, "SELECT 5", &q1);
  mock_finalize_fail = 1;
  c_orm_finalize_cached(&mock_db, q1); /* unmanaged now */
  mock_finalize_fail = 0;
  mock_finalize_fail = 0;

  /* Unmanaged query finalize fail when cache is ENABLED */
  c_orm_enable_statement_caching(&mock_db, 1);

  /* Create a fake unmanaged query */
  mock_prepare_fail = 0;
  mock_db.vtable->prepare(&mock_db, "SELECT 9", &q1);

  mock_mutex_unlock_fail = 1;
  c_orm_finalize_cached(&mock_db, q1); /* Unmanaged, unlock fails */
  mock_mutex_unlock_fail = 0;

  mock_db.vtable->prepare(&mock_db, "SELECT 10", &q1);
  mock_finalize_fail = 1;
  c_orm_finalize_cached(&mock_db, q1); /* Unmanaged, finalize fails */
  mock_finalize_fail = 0;
...
TEST test_cache_unlock_coverage_2(void) {
#if !defined(_WIN32) && !defined(_WIN64)
  c_orm_query_t *q1;
  setup_mock_db();
  c_orm_enable_statement_caching(&mock_db, 1);
