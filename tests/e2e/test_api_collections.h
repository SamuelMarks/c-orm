...
TEST test_api_find_all_generic_realloc(void) {
  ...
  g_step_count = 20;
  ASSERT_EQ(C_ORM_OK,
            c_orm_find_all_generic(&custom_db, &meta, &out_arr, &out_cnt));
...
TEST test_api_scatter_gather_realloc_and_err(void) {
  ...
  ASSERT_EQ(C_ORM_OK, c_orm_shard_manager_add_node(sm, 0, &db1));
  ASSERT_EQ(C_ORM_OK, c_orm_shard_manager_add_node(sm, 1, &db2));

  /* Shard 1 returns 20 items (cap starts at 16, so triggers total_count +
   * shard_count > total_cap) */
  g_step_count = 20;
  ASSERT_EQ(C_ORM_OK,
            c_orm_scatter_gather_generic(sm, &meta, &out_arr, &out_cnt));
  ASSERT_EQ(20, out_cnt);
  ASSERT(out_arr != NULL);
  free(out_arr);
  out_arr = NULL;
...
/* ========================================================================= */
/* --- Identity Map and Generic Coverage --- */
/* ========================================================================= */
...
TEST test_api_identity_map_and_generic_deep(void) {
  ...
  ASSERT_EQ(C_ORM_OK, c_orm_identity_map_init(&map));
  ASSERT_EQ(C_ORM_OK, c_orm_identity_map_get_or_set_str(
                          &map, &Users_meta, "key1", &val1, &out_ptr));
  ASSERT_EQ(&val1, out_ptr);
  /* Same key found */
  ASSERT_EQ(C_ORM_OK, c_orm_identity_map_get_or_set_str(
                          &map, &Users_meta, "key1", &val2, &out_ptr));
  ASSERT_EQ(&val1, out_ptr);
  /* Different key */
  ASSERT_EQ(C_ORM_OK, c_orm_identity_map_get_or_set_str(
                          &map, &Users_meta, "key2", &val2, &out_ptr));
  ...
  ASSERT_EQ(C_ORM_ERROR_MEMORY,
            c_orm_identity_map_get_or_set_str(&map, &Users_meta, "key_oom",
                                              &val1, &out_ptr));
  ...
  g_deep_alloc_cnt = 0;
  ASSERT_EQ(C_ORM_ERROR_MEMORY,
            c_orm_identity_map_get_or_set_str(&map, &Users_meta, "key_oom2",
                                              &val1, &out_ptr));
  ...
  ASSERT_EQ(C_ORM_ERROR_MEMORY, c_orm_identity_map_get_or_set_int(
                                    &map, &Users_meta, 9999, &val1, &out_ptr));
  ...
  /* 3. Generic CRUD query errors */
  {
    ...
    ASSERT_EQ(C_ORM_ERROR_UNKNOWN, c_orm_insert_generic(&gdb, &Users_meta, &u));
    /* get_generic step failure */
    ASSERT_EQ(C_ORM_ERROR_UNKNOWN, c_orm_get_generic(&gdb, &Users_meta, 1, &u));
    /* get_generic_string step failure */
    ASSERT_EQ(C_ORM_ERROR_UNKNOWN,
              c_orm_get_generic_string(&gdb, &Users_meta, "1", &u));
    gvt.step = mock_step;
...
TEST test_api_generic_and_scatter_gather_deep(void) {
  ...
  g_step_count = 20;
  ASSERT_EQ(C_ORM_ERROR_MEMORY, c_orm_find_all_generic(&custom_db, &Users_meta,
                                                       &out_arr, &out_cnt));
  c_orm_set_allocators(orig_m, orig_r, orig_f);
  ...
  ASSERT_EQ(C_ORM_ERROR_UNKNOWN, c_orm_find_all_generic(&custom_db, &Users_meta,
                                                        &out_arr, &out_cnt));
  custom_vt.step = mock_step_sequence;
  ...
  ASSERT_EQ(C_ORM_OK, c_orm_shard_manager_add_node(sm, 0, &fail_db));
  ASSERT_EQ(C_ORM_ERROR_UNKNOWN,
