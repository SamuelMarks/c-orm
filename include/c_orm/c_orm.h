/**
 * @file c_orm.h
 * @brief Abstract Object Relational Mapper for SQLite, PostgreSQL, and MySQL in C.
 ...
 */
typedef void (*c_orm_log_cb_t)(const char* query, double duration_ms, void* user_data);
 ...
 * @return 0 on success, non-zero on failure (e.g., -4 if dialect is not compiled in).
 */
 ...
 * Note: Under the hood, this relies on non-blocking native APIs or a thread pool.
 * @param db Database connection handle.
 * @param query SQL query string.
 ...
 * @param query The query builder handle.
 * @param sql_out Pointer to receive the dynamically allocated SQL string. Caller must free().
 * @return 0 on success, non-zero on error.
 */
